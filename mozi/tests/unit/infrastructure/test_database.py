...
class TestSQLiteSessionStorage:
    ...
    async def test_init_db_creates_tables(self, temp_db_path: str) -> None:
    ...
    async def test_save_and_load_session(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_load_nonexistent_session(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_delete_nonexistent_session(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_list_sessions_with_limit(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_save_and_load_message(self, storage: SQLiteSessionStorage) -> None:
        """Test saving and loading a message."""
        session = Session(id="msg-test-session")
        await storage.save_session(session)
        ...
        messages = await storage.load_messages("msg-test-session")
    ...
    async def test_load_messages_empty(self, storage: SQLiteSessionStorage) -> None:
        """Test loading messages for a session with no messages."""
        session = Session(id="empty-msg-session")
        await storage.save_session(session)

        messages = await storage.load_messages("empty-msg-session")
    ...
    async def test_update_session(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_session_status_transitions(self, storage: SQLiteSessionStorage) -> None:
    ...
    async def test_multiple_messages_order(self, storage: SQLiteSessionStorage) -> None:
        """Test that messages are loaded in correct order."""
        session = Session(id="order-test-session")
        await storage.save_session(session)
        ...
        messages = await storage.load_messages("order-test-session")
        assert len(messages) == 5
        for i, msg in enumerate(messages):
            assert msg.content == f"Message {i}"

    async def test_close_without_init(self) -> None:
        """Test closing storage without initialization."""
